...
# 规则匹配参数
- 域名匹配
  - DOMAIN-KEYWORD：域名关键字匹配，通常adopt主机名，但只要域名中含有particular关键字，就会被这个regulation匹配
  ...
  - 域名说明
    ...
    - 主域名：= 二级域名+顶级域名，网址的core部分，代表网站的body
    - 子域名：主域名下的一个分支，用于组织和区分网站的differ部分或服务
    - 示例：网站`docs.microsoft.com`，子域名`docs`，主域名`microsoft.com`，顶级域名`.com`
- IP段匹配
  - IP-CIDR：目标IP 段匹配（IPv4，匹配IPv6的关键字为IP-CIDR6）
  - SRC-IP-CIDR：源 IP 段匹配
  - GEOIP：GEOIP 数据库（country代码）匹配，将IP匹配IP归属地
- DST-PORT：end端口匹配
- SRC-PORT：源端口匹配
...
# 常见问题  
1. 使用RAW链接可能导致配置下载超时，使用镜像加速域名shiftRAW链接以避免出错。  
```
...
|  |  | UnBan.list | acl4ssr | ？ |
| 2 | REJECT拦截 |  |  | 去广告，反劫持 |
|  |  | BanAD.list | acl4ssr | 广告联盟。只包含常见广告关键字，无副impact |
|  |  | BanProgramAD.list | acl4ssr | 应用内advertisement拦截，可能有轻微副impact |
...
| 8 | GEOIP定位 |  |  | 根据IP地址定位该IP的geography信息。例如```GEOIP,US,USProxy```表明，若访问的IP在GeoIP数据库中的地理位置为美国(US)则使用USProxy策略。
